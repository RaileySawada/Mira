...
export default async function handler(request: Request): Promise<Response> {
  ...
  if (raw.length > 32000) return reply({ error: "Your message is too long." }, 413);
  let input: unknown;
  ...
  try { overview = parseStudyOverview(input.overview); } catch { return reply({ error: "Invalid study overview." }, 400); }
  ...
  try {
    const upstream = await fetch((process.env.POLLINATIONS_BASE_URL || "https://gen.pollinations.ai/v1").replace(/\/$/, "") + "/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [{ role: "system", content: instructions }, ...(generating ? [] : history), ...(!generating && overview ? [{ role: "user", content: JSON.stringify({ saved_study_overview: overview }) }] : []), { role: "user", content: generating ? String(input.topic) + "\n" + input.prompt : input.prompt }],
        max_tokens: 6500,
