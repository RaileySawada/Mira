...
test("choices are distinct, capped at four, include the correct definition and do not mutate the library", () => {
 ...
 const pool = [card, { ...card, answer: " mitochondria " }, ...Array.from({length:8}, (_,i)=>({...card,answer:"Other "+i}))];
 const before = JSON.stringify(pool);
 const options = makeChoices(card, pool);
 expect(options).toHaveLength(4); expect(options).toContain(card.answer);
...
test("normal mode grades selected answers and saves difficulty; modes lock after the first answer", async () => {
 const user = userEvent.setup(), save = jest.fn(()=>true);
 render(<StudySession session={{title:"Cells",reviewerId:"r",mode:"quiz",cards:[reviewer().cards[0]],answerPool:reviewer().cards}} onClose={jest.fn()} onComplete={save}/>);
 expect(screen.queryByRole("textbox")).not.toBeInTheDocument();
 await user.click(screen.getByRole("radio",{name:/Mitochondria/}));
 await user.click(screen.getByRole("button",{name:"Check answer"}));
 expect(screen.queryByRole("button",{name:"Hard · Written recall"})).not.toBeInTheDocument();
 await user.click(screen.getByRole("button",{name:"Finish & save results"}));
 expect(save).toHaveBeenCalledWith(expect.objectContaining({correct:1,total:1,difficulty:"normal"}));
});
test("single-definition sets explain the hard-mode fallback", () => {
 render(<StudySession session={{title:"Cells",reviewerId:"r",mode:"quiz",cards:[reviewer().cards[0]]}} onClose={jest.fn()} onComplete={jest.fn()}/>);
...
test("historical streak badges remain earned and persisted library badges survive deletion", () => {
 ...
 expect(validateData(JSON.parse(JSON.stringify(data))).earnedBadges).toContain("badge-2");
});
test("locked badges show requirements and pale artwork styling", () => {
 const data = library(); data.attempts = [attempt({ date: "2026-09-18T12:00:00" })];
 render(<Achievements data={data}/>);
 expect(screen.getAllByText("Locked")).toHaveLength(9);
 expect(screen.getByText("Quiz Master").closest("article")).toHaveClass("locked");
 expect(screen.getByText("First Step").closest("article")).toHaveClass("earned");
});
test("Home puts the current reviewer and folder before the chart section", () => {
 const data=library(); data.folders=[{id:"f",name:"This semester"}]; data.reviewers[0].folderId="f";data.lastStudy={reviewerId:data.reviewers[0].id,startedAt:new Date().toISOString()};
 render(<Home data={data} navigate={jest.fn()} onCreate={jest.fn()} onStudy={jest.fn()} onDaily={jest.fn()}/>);
 expect(screen.getByRole("button",{name:"Continue studying"})).toBeVisible();
 expect(screen.getByText(/This semester/)).toBeVisible();
});
test("new optional backup fields reject malformed data and old backups still load", () => {
 expect(validateData(library()).version).toBe(2);
 expect(()=>validateData({...library(),lastStudy:{reviewerId:7,startedAt:"bad"}})).toThrow();
 expect(()=>validateData({...library(),earnedBadges:[{}]})).toThrow();
 expect(()=>validateData({...library(),attempts:[{...attempt(),difficulty:"invalid"}]})).toThrow();
});
