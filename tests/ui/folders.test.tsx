...
test("moves reviewers and deleting a folder preserves its cards", async () => {
  ...
  expect(update.mock.calls[1][0].reviewers[0].cards).toEqual(
    data.reviewers[0].cards,
  );
...
test("1000 folders render only five rows, newest first, with searchable pages", () => {
  const data = library();
  data.folders = Array.from({ length: 1000 }, (_, index) => ({
    id: "f" + index,
    name: "Folder " + index,
  }));
  render(
  ...
  const nav = within(
    screen.getByRole("navigation", { name: "Reviewer folders" }),
  );
...
test("a newly created folder opens and clears search on the first page", async () => {
  const data = library();
  data.folders = Array.from({ length: 8 }, (_, index) => ({
    id: "f" + index,
    name: "Old " + index,
  }));
  ...
  expect(
    within(screen.getByRole("region", { name: "Folder contents" })).getByRole(
      "heading",
      { name: "Newest" },
    ),
  ).toBeVisible();
...
test("failed asynchronous folder saves preserve the editor and selection", async () => {
  let finish!: (saved: boolean) => void;
  const update = jest.fn(() => new Promise<boolean>((resolve) => { finish = resolve; }));
  render(<Folders data={library()} update={update} onEdit={jest.fn()} onStudy={jest.fn()} />);
  fireEvent.click(screen.getByRole("button", { name: "New folder" }));
  fireEvent.change(screen.getByLabelText("Folder name"), { target: { value: "Exam" } });
  fireEvent.click(screen.getByRole("button", { name: "Save folder" }));
  expect(screen.getByRole("dialog")).toBeInTheDocument();
  finish(false);
  await waitFor(() => expect(screen.getByText("Your folder changes could not be saved. Please try again.")).toBeInTheDocument());
  expect(screen.getByRole("dialog")).toBeInTheDocument();
  expect(screen.getByLabelText("Folder name")).toHaveValue("Exam");
});
