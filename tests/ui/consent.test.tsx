...
import { hasPolicyConsent } from "../../src/features/consent/policy";
...
test("first visit requires both agreements before entering the app", () => {
  render(<App />);
  expect(
    screen.getByRole("heading", { name: "Welcome to Mira." }),
  ).toBeVisible();
  ...
  fireEvent.click(
    screen.getByLabelText("I have read and agree to the privacy policy."),
  );
...
test("missing, malformed and outdated agreement records require acceptance", () => {
  ...
  for (const value of [
    ...
    JSON.stringify({ version: POLICY_VERSION, acceptedAt: "invalid" }),
  ]) {
