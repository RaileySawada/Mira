...
import { attempt } from "../support/fixtures";
...
const config = {
  ...
  other: { label: "Other", theme: { light: "red", dark: "blue" } },
};
const payload = [
  {
    graphicalItemId: "chart",
    dataKey: "total",
    name: "total",
    value: 3,
    color: "red",
...
test("chart styles support both themes and label-only configurations", () => {
  const { container, rerender } = render(
    <ChartStyle id="colors" config={config} />,
  );
  ...
  rerender(
    <ChartStyle
      id="partial"
      config={{ value: { theme: { light: "red", dark: "" } } }}
    />,
  );
...
test("tooltips support labels, formatted values and indicators", () => {
  const { rerender, container } = render(
    wrapper(<ChartTooltipContent active payload={payload} label="total" />),
  );
  ...
  rerender(
    wrapper(
      <ChartTooltipContent
        ...
        hideLabel
        hideIndicator
        formatter={(value) => "Answered " + value}
      />,
    ),
  );
  ...
  rerender(
    wrapper(
      <ChartTooltipContent
        active
        payload={[
          { graphicalItemId: "chart", name: "unknown", value: "answer" },
        ]}
        nameKey="unknown"
      />,
    ),
  ...
  rerender(
    wrapper(
      <ChartTooltipContent
        active
        payload={[
          { graphicalItemId: "chart", type: "none", value: 0 },
          { graphicalItemId: "chart", value: undefined },
        ]}
      />,
    ),
  );
...
test("legend and tooltip resolve nested names and custom icons", () => {
  ...
  const { rerender } = render(
    <ChartContainer config={customConfig}>
      <div>
        ...
        <ChartLegendContent
          payload={[{ value: "total", dataKey: "total", color: "red" }]}
        />
      </div>
    </ChartContainer>,
  );
  ...
  rerender(
    wrapper(
      <ChartLegendContent
        payload={[
          { value: "total", dataKey: "total", color: "red" },
  ...
  rerender(
    wrapper(
      <ChartLegendContent
        payload={[{ value: "nested", payload: { kind: "other" } }]}
        nameKey="kind"
      />,
    ),
  );
