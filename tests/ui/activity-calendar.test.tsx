...
test("calendar groups local days and shows weighted day summaries", () => {
  ...
  render(
    <ActivityCalendar
      attempts={[
        attempt({ id: "one", date: today.toISOString(), correct: 1, total: 2 }),
        attempt({
          id: "two",
  ...
  fireEvent.click(
    screen.getByRole("button", { name: label + ": 2 completed, Active" }),
  );
...
test("calendar navigates month/year boundaries and opens empty days", () => {
  ...
  expect(
    screen.getByRole("heading", {
      name: december.toLocaleDateString(undefined, {
        month: "long",
        year: "numeric",
      }),
    }),
  ...
  fireEvent.click(
    screen.getByRole("button", {
      name:
        january.toLocaleDateString(undefined, { dateStyle: "full" }) +
        ": 0 completed, No activity",
    }),
  );
...
test("activity icons distinguish one completion from five", () => {
  ...
  expect(
    screen.getByRole("button", { name: /1 completed, Slightly active/ }),
  ).toBeVisible();
  view.rerender(
    <ActivityCalendar
      attempts={Array.from({ length: 5 }, (_, i) =>
        attempt({ id: String(i), date }),
      )}
    />,
  );
  expect(
    screen.getByRole("button", { name: /5 completed, Super active/ }),
  ).toBeVisible();
});
