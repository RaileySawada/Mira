...
test("daily review saves a result into the application library", async () => {
  ...
  await user.click(
    await screen.findByRole("button", { name: "Keep learning" }),
  );
  await user.click(
    await screen.findByRole("button", { name: "Back to learning" }),
  );
...
test("reviewer page starts flashcards and individual quizzes", async () => {
  ...
  await waitFor(() => expect(screen.getByText("MULTIPLE CHOICE")).toBeVisible());
  await user.click(screen.getByLabelText("Close dialog"));
});
...
test("reviewer quizzes include all cards even when the daily question limit is small", async () => {
  const user = userEvent.setup();
  history.replaceState(null, "", "/reviewers");
  const data = library();
  data.settings.dailyQuizSize = 1;
  data.settings.quizSize = 1;
  data.reviewers[0].cards = Array.from({ length: 15 }, (_, i) => ({
    id: "card-" + i,
    question: "Question " + i,
    answer: "Answer " + i,
  }));
  ...
  render(<App />);
  await user.click(screen.getByRole("button", { name: "Take quiz" }));
  await waitFor(() => expect(screen.getByText("1 / 15")).toBeVisible());
});
