...
beforeEach(() => {
  ...
  jest
    .mocked(signInAnonymously)
    .mockResolvedValue({ user: { uid: "visitor" } } as Awaited<
      ReturnType<typeof signInAnonymously>
    >);
  jest
    .mocked(onDisconnect)
    .mockReturnValue({ remove: disconnectRemove } as unknown as ReturnType<
      typeof onDisconnect
    >);
  jest.mocked(onValue).mockImplementation(((
    path: string,
...
test("registers disconnect cleanup before writing and waits for count snapshot", async () => {
  ...
  await flush();
  expect(disconnectRemove.mock.invocationCallOrder[0]).toBeLessThan(
    jest.mocked(set).mock.invocationCallOrder[0]!,
  );
...
test("permission failures cannot be overwritten by late connection events", async () => {
  ...
  expect(notify).toHaveBeenLastCalledWith(
    expect.objectContaining({ status: "unavailable", count: null }),
  );
...
test("indicator displays counts and errors and hides offline", () => {
  const { rerender } = render(
    <OnlineCount presence={{ status: "online", count: 3 }} />,
  );
