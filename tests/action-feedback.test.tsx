...
test("actions show loading, prevent duplicates, show success and return to idle", async () => {
  ...
  act(() => { void result.current.run(action, complete); });
...
test("failed and cancelled actions never show success and can be retried", async () => {
  ...
  await act(async () => { await result.current.run(() => { throw new Error("Storage full"); }); });
  expect(result.current.error).toBe("Storage full");
  await act(async () => { await result.current.run(() => { throw "failed"; }); });
  expect(result.current.error).toMatch(/could not/);
...
test.each(["resolve", "reject"])("ignores late %s after unmount", async mode => {
  const { result, unmount } = renderHook(useActionFeedback);
  let resolve!: () => void, reject!: () => void;
  const pending = new Promise<void>((done, fail) => { resolve = done; reject = () => fail(new Error("Late")); });
  act(() => { void result.current.run(() => pending); });
  unmount();
  await act(async () => { if (mode === "resolve") resolve(); else reject(); });
});
...
test("process button exposes progress and success without changing its accessible action name", () => {
  ...
  rerender(<ProcessButton label="Save" state="idle" disabled>Save changes</ProcessButton>);
...
test("opening selection keeps focus on the trigger and supports keyboard choice", () => {
  ...
  const { rerender } = render(<SearchSelect label="Topic" value="a" options={[{ value: "a", label: "A" }, { value: "b", label: "B" }]} onChange={change} />);
  ...
  rerender(<SearchSelect label="Topic" value="" options={[]} onChange={change} />);
  fireEvent.click(trigger);
  fireEvent.keyDown(trigger, { key: "Enter" });
...
test("Escape from the search field closes the dropdown and restores trigger focus", () => {
  render(<SearchSelect label="Topic" value="a" options={[{ value: "a", label: "A" }]} onChange={jest.fn()} />);
