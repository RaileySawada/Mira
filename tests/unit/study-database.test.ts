...
test("question snapshots and schedules round trip in order and deletions remove stale rows",async()=>{
 const data=library();data.attempts=[{id:"a",reviewerId:"reviewer-1",title:"Quiz",mode:"quiz",date:"2026-09-20T12:00:00Z",total:1,correct:1,results:[{cardId:"card-1",question:"q",expectedAnswer:"a",userAnswer:"a",correct:true,durationMs:50}]}];data.schedules=[{reviewerId:"reviewer-1",cardId:"card-1",lastReviewedAt:"2026-09-20T12:00:00Z",nextReviewAt:"2026-09-21T12:00:00Z",repetitions:1,lapses:0,intervalDays:1,recent:[true],source:"quiz"}];
 await writeStudyDatabase(db,data);const saved=await readStudyDatabase(db);expect(saved?.attempts[0].results).toEqual(data.attempts[0].results);expect(saved?.schedules).toEqual(data.schedules);
 await writeStudyDatabase(db,{...data,reviewers:[],attempts:[],schedules:[]});expect((await readStudyDatabase(db))?.reviewers).toEqual([]);expect((await readStudyDatabase(db))?.attempts).toEqual([]);
});
test("an aborted transaction cannot partially replace the library",async()=>{
 const saved=await migrateStudyDatabase(db,library());const original=db.transaction.bind(db);
 const spy=jest.spyOn(db,"transaction").mockImplementation(((...args:Parameters<IDBDatabase["transaction"]>)=>{const tx=original(...args);queueMicrotask(()=>tx.abort());return tx;}) as IDBDatabase["transaction"]);
 await expect(writeStudyDatabase(db,{...saved,reviewers:[]})).rejects.toThrow();spy.mockRestore();expect(await readStudyDatabase(db)).toEqual(saved);
});

 test("recovery export includes the raw library and unfinished draft", async () => {
 await migrateStudyDatabase(db, library()); await writeQuizDraft(db, { id: "paused" });
 const recovery = await exportStudyRecovery(db); expect(recovery.drafts).toEqual([{id:"paused"}]); expect(recovery.cards).toHaveLength(2); expect(recovery.metadata).toHaveLength(1);
 });
