...
test.each(["reviewers", "topics", "quizzes"] as const)("remembers %s layout after remount", page => {
...
test("page preferences are independent and invalid values fall back to grid", () => {
  ...
  expect(renderHook(() => useViewPreference("reviewers")).result.current.view).toBe("list");
  expect(renderHook(() => useViewPreference("topics")).result.current.view).toBe("grid");
  expect(renderHook(() => useViewPreference("quizzes")).result.current.view).toBe("grid");
});
