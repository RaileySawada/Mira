...
test("activity table searches, sorts, and paginates every result", () => {
  const data = library();
  data.attempts = Array.from({ length: 10 }, (_, index) => attempt({
    id: "attempt-" + index,
  ...
  fireEvent.change(screen.getByLabelText("Search activity"), { target: { value: "Algebra" } });
  expect(screen.getByText("1 result")).toBeVisible();
  expect(screen.getByText("Algebra")).toBeVisible();
  fireEvent.change(screen.getByLabelText("Search activity"), { target: { value: "" } });
  fireEvent.click(screen.getByRole("button", { name: /Reviewer/ }));
  expect(screen.getByRole("columnheader", { name: /Reviewer/ })).toHaveAttribute("aria-sort", "ascending");
...
test("reviewers and topics can switch between grid and list layouts", () => {
  ...
  const { rerender } = render(<Reviewers data={data} onCreate={jest.fn()} onEdit={jest.fn()} onDelete={jest.fn()} onStudy={jest.fn()} onQuiz={jest.fn()} />);
