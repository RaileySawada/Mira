...
test("voice input updates an editable draft, replaces interim text, and cleans up", () => {
  ...
  act(() => recognition.onresult?.({ results: [Object.assign([{ transcript: "cell" }], { isFinal: true })] }));
  act(() => recognition.onresult?.({ results: [Object.assign([{ transcript: "cell biology" }], { isFinal: true })] }));
...
test("dictation never submits and can be edited before sending", () => {
  ...
  act(() => RecognitionMock.current.onresult?.({ results: [Object.assign([{ transcript: "Explain cells" }], { isFinal: true })] }));
...
test("continues across pauses without losing text and cancels restart on Stop", () => {
  ...
  try {
    ...
    act(() => recognition.onresult?.({ results: [Object.assign([{ transcript: "cells" }], { isFinal: true })] }));
    ...
    act(() => resumed.onresult?.({ results: [Object.assign([{ transcript: "and DNA" }], { isFinal: true })] }));
...
test("late callbacks from a stopped recording cannot overwrite a new recording", () => {
  ...
  act(() => { lateResult?.({ results: [Object.assign([{ transcript: "stale" }], { isFinal: true })] }); lateEnd?.(); });
...
test("interim guesses never get duplicated across automatic restarts", () => {
  ...
  try {
    ...
    const old = RecognitionMock.current;
    const lateResult = old.onresult;
    act(() => old.onresult?.({ results: [speech("cell", true), speech("biology", false)] }));
    act(() => old.onend?.());
...
test("intentional repeated words are preserved and removed interim guesses disappear", () => {
  ...
  act(() => result.current.start(""));
  const current = RecognitionMock.current;
  act(() => current.onresult?.({ results: [speech("very very important", true), speech("guess", false)] }));
  act(() => current.onresult?.({ results: [speech("very very important", true)] }));
