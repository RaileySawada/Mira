...
export function InstallPrompt() {
  ...
  useEffect(() => {
    if (offered.current) {
      try {
        ...
        /* The mounted prompt still appears only once. */
      }
    }
  ...
  return (
    <aside className="install-prompt" role="region" aria-label="Install Mira">
      <div>
        <strong>Keep Mira close.</strong>
        <p>
          {manual
            ? "Open your browser’s Share menu, choose Add to Home Screen, then Add."
            : "Install Mira for quick access and offline studying."}
        </p>
      ...
      <div className="flex shrink-0 gap-2">
        <button
          ...
          disabled={install.state === "loading"}
          onClick={() => setVisible(false)}
        >
          Not now
        </button>
        <ProcessButton
          label={manual ? "Got it" : "Install Mira"}
          state={install.state}
          onClick={() => {
            if (!event.current) {
