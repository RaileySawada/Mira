...
export function OnlineCount({ presence }: { presence?: PresenceState }) {
  ...
  return (
    <div
      className={"online-count " + (available ? "is-online" : "")}
      role="status"
      title={
        presence.message ??
        "Connected browsers right now. Tabs in the same browser count once; separate devices count separately."
      }
    >
      <span aria-hidden="true" className="online-count-dot" />
      <span>
        {available
          ? (presence.count ?? 0).toLocaleString() + " online"
