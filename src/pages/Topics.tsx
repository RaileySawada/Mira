    ...
    <>
      ...
      <PageHeading
        ...
        action={
          <div className="page-heading-actions">
            <ViewToggle value={view} onChange={setView} label="Topic layout" />
            <button
      ...
      {data.topics.length ? (
        <div
          className={
            view === "grid"
              ? "topic-collection topic-grid"
              : "topic-collection topic-list"
          }
        >
          {data.topics.map((topic) => {
            const count = data.reviewers.filter(
              (reviewer) => reviewer.topicId === topic.id,
            ).length;
            return (
              <article key={topic.id} className="panel topic-card">
                <span
                    ...
                    onClick={async () => {
                      if (
                        await confirmAction(
                          "Delete “" +
                            topic.name +
                            "”? Its reviewers will move to Uncategorized.",
                        )
                      )
                        update({
                          ...data,
