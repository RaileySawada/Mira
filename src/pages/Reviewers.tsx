...
export function Reviewers({
  ...
  const reviewers = data.reviewers.filter(
    (reviewer) =>
      (reviewer.title + " " + reviewer.description)
        .toLowerCase()
        .includes(search.toLowerCase()) &&
      (topic === "all" || reviewer.topicId === topic) &&
      (folder === "all" || (reviewer.folderId ?? "") === folder),
  );
  return (
    <>
      {layoutError && (
      ...
      <div className="reviewer-toolbar mb-6 flex items-start gap-2 sm:gap-3">
        ...
        <SearchSelect
          ...
          triggerIcon={<Icon name="Topics" size={18} />}
        ...
        <button
          className="button primary toolbar-create"
          onClick={onCreate}
          aria-label="New reviewer"
        >
          <Icon name="plus" size={16} />
          <span className="hidden sm:inline">New reviewer</span>
        </button>
      </div>
      ...
      {reviewers.length ? (
        <div
          className={
            view === "grid"
              ? "reviewer-collection reviewer-grid"
              : "reviewer-collection reviewer-list"
          }
        >
          {reviewers.map((reviewer) => {
            const currentTopic = data.topics.find(
              (item) => item.id === reviewer.topicId,
            );
            return (
              <article className="panel reviewer-card" key={reviewer.id}>
                <div className="reviewer-card-main">
                  <span className="badge">
                    <span
                      className="mr-1.5 inline-block h-1.5 w-1.5 rounded-full"
                      style={{
                        backgroundColor: currentTopic?.color || "#a8a29e",
                      }}
                    />
                    {currentTopic?.name || "Uncategorized"}
                  </span>
                  ...
                  <small>
                    {reviewer.cards.length} flashcards · Updated{" "}
