...
export function Activity({ data }: { data: StudyData }) {
  ...
  return (
    <>
      ...
      />
      <p className="mb-3 text-sm">{studyStreak(data)} day study streak</p>
      <ActivityCalendar
        ...
        <section className="activity-table panel">
          ...
          <div className="data-table-scroll">
            <table>
              ...
              <tbody>
                {visible.map((attempt) => (
                  <tr key={attempt.id} className="data-table-row">
                    <td className="activity-title">{attempt.title}</td>
                    <td>
                      <span className="activity-type">
                        {attempt.mode === "daily" ? "Daily review" : "Quiz"}
                      </span>
                    </td>
                    ...
                    <td>
                      <span className="badge">{result(attempt)}%</span>
          ...
          {attempts.length ? (
            ...
            <p className="data-table-empty">No activity matches “{search}”.</p>
          )}
        </section>
      ) : (
        <EmptyState
  ...
  return (
    <th aria-sort={selected ? direction : "none"}>
      <button type="button" onClick={() => onSort(column)}>
        {label}
        <Icon
          name={selected && direction === "ascending" ? "up" : "down"}
          size={14}
        />
      </button>
    </th>
