...
export function Quizzes({
  ...
  const done = data.attempts.some(
    (attempt) =>
      attempt.mode === "daily" && dayKey(attempt.date) === dayKey(new Date()),
  );
  return (
    <>
      {layoutError && (
        <p role="status" className="mb-4 text-xs text-stone-500">
          {layoutError}
        </p>
      )}
      <PageHeading
        ...
        description="Pick a reviewer to focus on, or mix things up with a daily review."
      />
      <section className="quiz-daily-card">
        <div>
          <span className="quiz-daily-status">
            {done ? "Completed today" : "A little practice, every day"}
          </span>
          <h2>Your daily review</h2>
          <p>
            {done
              ? "You’ve completed today’s review. Come back tomorrow or keep practicing."
              : "A mix of questions from your saved reviewers. Take it at your own pace."}
          </p>
        </div>
        <div className="quiz-daily-action">
          ...
          <small>
            {Math.min(
              ...
              available.reduce(
                (total, reviewer) => total + reviewer.cards.length,
                0,
              ),
            )}{" "}
      ...
      <div className="collection-heading">
        ...
        {available.length > 0 && (
          <ViewToggle value={view} onChange={setView} label="Quiz layout" />
        )}
      </div>
      {available.length ? (
        <div
          className={
            view === "grid"
              ? "quiz-collection quiz-grid"
              : "quiz-collection quiz-list"
          }
        >
            ...
            <article className="panel quiz-card" key={reviewer.id}>
              <div className="quiz-card-main">
                <span className="quiz-topic">
                  {data.topics.find((topic) => topic.id === reviewer.topicId)
                    ?.name ?? "Uncategorized"}
                </span>
