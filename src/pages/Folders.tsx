...
export function Folders({
  ...
  data: StudyData;
  update: (next: StudyData) => boolean | Promise<boolean>;
  ...
  useEffect(() => {
    ...
    const outside = (event: PointerEvent) => {
      if (
        ...
        !folderPicker.current?.contains(event.target)
      )
  ...
  const reviewers = data.reviewers.filter(
    (reviewer) => (reviewer.folderId ?? "") === selected,
  );
  ...
  async function persist(next: StudyData) {
    ...
    setError(
      saved ? "" : "Your folder changes could not be saved. Please try again.",
    );
    return saved;
  }
  return (
    <>
      <PageHeading
        eyebrow="YOUR LIBRARY, ORGANIZED"
        title="Your folders."
        description="Keep your reviewers organized by course, exam, or project."
        action={
          <button
      ...
      <div className="folder-layout">
        <div
          ...
          onKeyDown={(event) => {
            if (event.key === "Escape") {
              setFoldersOpen(false);
              event.currentTarget
                .querySelector<HTMLButtonElement>(".folder-picker-trigger")
                ?.focus();
            }
          }}
          onBlur={(event) => {
        ...
        >
          ...
          >
            ...
            <span>{selectedFolder?.name ?? "Unfiled"}</span>
            <ChevronDown size={16} aria-hidden="true" />
          </button>
          <nav
            id="folder-navigation"
            data-open={foldersOpen}
            aria-label="Reviewer folders"
            className="panel folder-navigation"
          >
            ...
            <p className="folder-result-count" role="status">
            ...
            >
              Unfiled <span>{counts.get("") ?? 0}</span>
            </button>
            {selectedFolder &&
              !visible.some((folder) => folder.id === selected) && (
                <div className="folder-pinned">
                  ...
                  >
                    ...
                    <span>{counts.get(selected) ?? 0}</span>
                  </button>
                </div>
              )}
            {visible.map((folder) => (
              <div key={folder.id} className="folder-row">
                ...
                <button
                  ...
                  onClick={async () => {
                    if (
                      !(await confirmAction(
                        "Delete this folder? Its reviewers will move to Unfiled.",
                      ))
                    )
                      return;
                    if (
                      (await persist({
                        ...
                        reviewers: data.reviewers.map((r) =>
                          r.folderId === folder.id ? { ...r, folderId: "" } : r,
                        ),
                      })) &&
                      selected === folder.id
                    )
            ...
            {!filtered.length && (
              <p className="folder-no-results">
                {search.trim()
                  ? "No matching folders. Try another name."
                  : "Create a folder to organize your reviewers."}
              </p>
            )}
            {pages > 1 && (
        ...
        <section className="min-w-0" aria-label="Folder contents">
          ...
          {reviewers.length ? (
            <div className="folder-reviewers">
              {reviewers.map((reviewer) => (
                <article className="panel folder-reviewer" key={reviewer.id}>
                  <div className="folder-reviewer-info">
                    <h3>{reviewer.title}</h3>
                    <p>{reviewer.cards.length} cards</p>
                  </div>
                  <div className="folder-reviewer-tools">
                    <SearchSelect
                      label={"Move " + reviewer.title + " to folder"}
                      value={reviewer.folderId ?? ""}
                      onChange={(folderId) =>
                        persist({
                          ...data,
                          reviewers: data.reviewers.map((r) =>
                            r.id === reviewer.id ? { ...r, folderId } : r,
                          ),
                        })
                      }
                      options={[
                    ...
                    <button
                      className="button primary"
                      disabled={!reviewer.cards.length}
            ...
            <EmptyState
              title="This folder is ready for your notes"
              description="Move reviewers here from Unfiled, or choose this folder when creating or editing a reviewer."
            />
          )}
        </section>
      </div>
