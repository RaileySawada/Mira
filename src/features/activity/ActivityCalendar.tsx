...
function activityLevel(count: number) {
  return count >= 5 ? 3 : count >= 2 ? 2 : count === 1 ? 1 : 0;
}
...
export function ActivityCalendar({
  ...
  const entries = selected
    ? [...(grouped.get(dayKey(selected)) ?? [])].sort(
        (a, b) => new Date(a.date).getTime() - new Date(b.date).getTime(),
      )
    : [];
  const practices = selected
    ? studyDates.filter((date) => dayKey(date) === dayKey(selected))
    : [];
  const total = entries.reduce((sum, entry) => sum + entry.total, 0);
  return (
    <>
      <section
        className="activity-calendar panel"
        aria-label="Study activity calendar"
      >
        <header className="calendar-toolbar">
          <div>
            <p className="eyebrow">YOUR STUDY DAYS</p>
            <h2 aria-live="polite">
        ...
        <div className="calendar-days">
          {days.map((date) => {
            ...
            const count =
              ...
              studyDates.filter((date) => dayKey(date) === key).length;
              ...
              >
                ...
                {Indicator && (
                  <span className="calendar-indicator">
                    <Indicator size={17} aria-hidden="true" />
                    <span>
                      {count}
        ...
        <div className="calendar-legend">
          {levels.slice(1).map((level, index) => {
            const Indicator = level.icon!;
            return (
              <span key={level.label} data-level={index + 1}>
                <Indicator size={15} aria-hidden="true" />
                {level.label} · {index === 0 ? "1" : index === 1 ? "2–4" : "5+"}
              </span>
            );
          })}
        </div>
        <p className="calendar-note">
          Completed quizzes, daily reviews and flashcard sessions, grouped by
          your device’s local date. Select any day for its summary.
        </p>
      </section>
      {selected && (
        <Modal
          title={selected.toLocaleDateString(undefined, { dateStyle: "full" })}
          onClose={() => setSelected(null)}
        >
          <p className="text-sm text-stone-500">
            {levels[activityLevel(entries.length + practices.length)].label}
          </p>
          {practices.length > 0 && (
            <p className="my-3 text-sm">
              {practices.length} completed flashcard session
              {practices.length === 1 ? "" : "s"}. Self-ratings affect
              scheduling, not quiz accuracy.
            </p>
          )}
          {entries.length ? (
            <>
              <div className="day-summary-stats">
                ...
                <div>
                  <strong>{percentage(entries)}%</strong>
                  <span>Accuracy</span>
                </div>
              </div>
                ...
                {entries.map((entry) => (
                  <li key={entry.id}>
                    <div>
                        ...
                        <details>
                          ...
                          {entry.results.map((result, index) => (
                            <div className="mt-3 text-sm" key={index}>
                              ...
                              <p>Expected: {result.expectedAnswer}</p>
                              <p>
                      ...
                      <p>
                        {entry.mode === "daily" ? "Daily review" : "Quiz"} ·{" "}
            ...
            <p className="mt-5 text-sm">
              No completed quizzes or daily reviews were saved on this day.
              Flashcard sessions appear separately above when completed.
            </p>
          )}
        </Modal>
      )}
