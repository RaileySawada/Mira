      ...
      {mode === "chat" ? <section className="ai-chat-main">
        ...
        <div className="chat-composer-wrap">
          ...
          <form onSubmit={submit} className="chat-composer">
            <label className="chat-input-label"><span className="sr-only">Your study question</span><textarea value={prompt} onChange={event => setPrompt(event.target.value)} required maxLength={3000} disabled={busy} placeholder="Message Mira…" rows={1} /></label>
            <ProcessButton label="Send question" state={busy ? "loading" : succeeded ? "success" : "idle"} successLabel="Ready" className="button primary chat-send" disabled={!prompt.trim()}>{busy ? undefined : <ArrowUp size={19} aria-hidden="true" />}</ProcessButton>
          </form>
        ...
        <div className="generator-intro"><span className="chat-eyebrow">CREATE A STUDY SET</span><h3>Make a review set in one go.</h3><p>Mira creates structured reviewers with five flashcards each. You can inspect every card before saving.</p></div>
