...
export function countConnections(value: unknown): number {
  ...
  return Object.values(value).filter(
    (connections) =>
      ...
      Object.values(connections).some((connected) => connected === true),
  ).length;
}

export function startPresence(
  notify: (state: PresenceState) => void,
): () => void {
  ...
  const reportError = () => {
    ...
    notify({
      ...
      count: null,
      message:
        "Live count unavailable. Check Firebase Anonymous sign-in, database URL and presence rules, then reload.",
    });
  };
      ...
      onValue(
        ...
        (snapshot) => {
          ...
          void (async () => {
            ...
            await set(node, true);
            if (stopped || connection !== generation) {
              await remove(node);
              return;
            }
            connected = true;
  ...
  return () => {
    ...
    if (ownRef)
      void remove(ownRef).catch(() => {
        /* Server onDisconnect remains registered. */
      });
    goOffline(database);
  };
}
